...
function sb_cl_render_sibling($type='next') {
	...
	if ($post->post_parent) {
		...
		if ($siblings = $wpdb->get_results($sql)) {
			...
			if ($type == 'prev' && $last) {
				$html .= '<span class="sb_cl_prev_page sb_cl" title="' . $last->post_title . '"><a href="' . get_permalink($last->ID) . '"><i class="fa fa-arrow-left"></i></a></span>';
			} else if ($type == 'next' && $next) {
				$html .= '<span class="sb_cl_next_page sb_cl" title="' . $next->post_title . '"><a href="' . get_permalink($next->ID) . '"><i class="fa fa-arrow-right"></i></a></span>';
			}
		}
	}
